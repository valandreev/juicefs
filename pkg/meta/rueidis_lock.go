...
func (m *rueidisMeta) Flock(ctx Context, inode Ino, owner uint64, ltype uint32, block bool) syscall.Errno {
	if m.compat == nil {
		return m.redisMeta.Flock(ctx, inode, owner, ltype, block)
	}
...
// Getlk retrieves information about a POSIX lock.
func (m *rueidisMeta) Getlk(ctx Context, inode Ino, owner uint64, ltype *uint32, start, end *uint64, pid *uint32) syscall.Errno {
	if m.compat == nil {
		return m.redisMeta.Getlk(ctx, inode, owner, ltype, start, end, pid)
	}
	...
	lkey := m.ownerKey(owner)
	owners, err := m.compat.HGetAll(ctx, m.plockKey(inode)).Result()
	if err != nil {
		return errno(err)
	}
...
// Setlk sets or removes a POSIX lock.
func (m *rueidisMeta) Setlk(ctx Context, inode Ino, owner uint64, block bool, ltype uint32, start, end uint64, pid uint32) syscall.Errno {
	if m.compat == nil {
		return m.redisMeta.Setlk(ctx, inode, owner, block, ltype, start, end, pid)
	}
...
// ListLocks retrieves all locks for a given inode.
func (m *rueidisMeta) ListLocks(ctx context.Context, inode Ino) ([]PLockItem, []FLockItem, error) {
	if m.compat == nil {
		return m.redisMeta.ListLocks(ctx, inode)
	}

	fKey := m.flockKey(inode)
	pKey := m.plockKey(inode)

	rawFLocks, err := m.compat.HGetAll(ctx, fKey).Result()
	if err != nil {
		return nil, nil, err
	}
	...
	rawPLocks, err := m.compat.HGetAll(ctx, pKey).Result()
	if err != nil {
		return nil, nil, err
	}
...
// Helper function to check if an error is a Nil error (for Rueidis compatibility).
func isNilErr(err error) bool {
	if err == nil {
		return false
	}
	errStr := err.Error()
	return errStr == "redis nil message" || errStr == "redis: nil" || strings.Contains(errStr, "redis nil")
}
