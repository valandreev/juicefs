...
|`--network-interfaces=value` <VersionAdd>1.4</VersionAdd>|用于 IP 发现的网络接口白名单；仅使用指定的接口，逗号分隔（例如 `eth0,en0`），为空表示使用所有接口。|
...
|`--writeback-threshold-size=0` <VersionAdd>1.4</VersionAdd>|启用 `--writeback` 后，仅小于此大小的数据块会暂存到本地缓存，较大的数据块将直接上传。默认为 0，表示所有数据块都暂存。|
